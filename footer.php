<div class="d-lg-none text-center px-3 py-3" class="object-fit:cover;">
    <img src="assets/logo/Logo Medan Oriental Stars.png" class="footer-logo-small">
</div>
<div class="mx-auto py-3 footer-wrapper">
    <div class="flex-column no-padding mb-none" style="display:flex;">
        <img src="assets/logo/Logo Medan Oriental Stars.png" class="footer-logo">
    </div>
    <!-- <div class="d-flex flex-column no-padding">
        <p class="footer-header">
            <a href="#">Home</a>
        </p>
    </div> -->
    <div class="d-flex flex-column no-padding">
        <p class="footer-header">
            <a href="#">Models</a>
        </p>
        <ul>
            <li>
                <a href="#">Tiggo 7 Pro</a>
            </li>
            <li>
                <a href="#">Tiggo 8 Pro</a>
            </li>
        </ul>
    </div>
    <div class="d-flex flex-column no-padding">
        <p class="footer-header">
            <a href="#">Price List</a>
        </p>
    </div>
    <div class="d-flex flex-column no-padding">
        <p class="footer-header">
            <a href="#">Promo</a>
        </p>
    </div>
    <div class="d-flex flex-column no-padding">
        <p class="footer-header">
            <a href="#">Layanan</a>
        </p>
        <ul>
            <li>
                <a href="#">Cari Dealer</a>
            </li>
            <li>
                <a href="#">Sparepart</a>
            </li>
        </ul>
    </div>
    <div class="d-flex flex-column no-padding">
        <p class="footer-header">
            <a href="#">Berita</a>
        </p>
    </div>
    <div class="d-flex flex-column no-padding">
        <p class="footer-header">
            <a href="#">Tentang Kami</a>
        </p>
        <ul>
            <li>
                <a href="#">With Chery with love</a>
            </li>
            <li>
                <a href="#">Tentang Chery</a>
            </li>
        </ul>
    </div>
</div>
<div class="sosmed d-flex centered mt-3 no-padding">
    <ul>
        <li>
            <a href="https://www.instagram.com/cherymedan/" target="_blank">
                <span class="fa-stack">
                    <i class="fa-solid fa-circle fa-stack-2x"></i>
                    <i class="fa-solid fa-brands fa-instagram fa-stack-1x fa-inverse"></i>
                </span>
            </a>
        </li>
        <li>
            <a href="#">
                <span class="fa-stack">
                    <i class="fa-solid fa-circle fa-stack-2x"></i>
                    <i class="fa-solid fa-brands fa-facebook-f fa-stack-1x fa-inverse"></i>
                </span>
            </a>
        </li>
    </ul>
</div>
<div class="copyright text-center">
    © 2022 Chery Oriental Group. All rights reserved
</div>